...
const Model: FC<ModelProps> = (props: ModelProps) => {
  ...
  const [currentAction, setCurrentAction] = useState(animationUrl);
  ...
  useEffect(() => {
    if (actions["Landing"] && actions[animationUrl]) {
      setTimeout(() => {
        var landingAction = animationMixer.clipAction(actions["Landing"]);
        var idleAction = animationMixer.clipAction(actions[animationUrl]);
